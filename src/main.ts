...
getusermedia({ audio: true }, function (err, stream) {
  ...
  window.onclick=()=>(document.getElementById('sound') as HTMLAudioElement).play();
  window.onmousedown=()=>(document.getElementById('sound') as HTMLAudioElement).play();
  window.vol = [0];
  for (var i = 0; i < BUFFER_SIZE; i++) {
    window.vol.push(0);
  }
  if (err) {
    // alert("need media permision to run")
  }
